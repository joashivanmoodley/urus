...
//
// $Id: sfHistoryManager.js 121880 2011-01-14 21:52:02Z svn $
//

if ("undefined" == typeof window.SFHistoryManager) {
     ...
     * For example:
     * <pre>
     * SFHistoryManager.addEventListener("history", {handleEvent:function(evt) {
     *     alert("History changed to: "+evt.token);
     * }});
     * </pre>
     */
    window.SFHistoryManager = (function() {
        var historyToken = "";
        ...
        function hashCheckTimer() {
            ...
            if (token != historyToken) {
                historyToken = token;
                window.SFHistoryManager.dispatch("history", {token:historyToken});
            }
        }
        ...
        function navigateFrame(/*String*/ token) {
            ...
            doc.open();
            doc.write('<html><body onload="parent.SFHistoryManager._onFrameLoad(token.innerText)"><div id="token">'+
        ...
        return set(new EventTarget(), {
            ...
            getUrlParams: function() {
                ...
                var href = location.href, i, arrLen;
                var index = href.indexOf('?');
                var paramsArr = [], singleParamArr = [];
                var params = {};
                
                if(hashIndex > index) {
                    paramsArr = index > 0 ? href.substring(index+1, hashIndex).split('&') : [];
                } else {
                    paramsArr = index > 0 ? href.substring(index+1).split('&') : [];
                }
                
                var arrLen = paramsArr.length;
                for(i=0; i<arrLen; i++) {
                    singleParamArr = paramsArr[i].split('=');
