/**
 * Abstract class for all single select components.
 ...
 */
function SFAbstractSingleSelect(modelOrArray) {
    //Do not allow this component to be instantiated.
    assert(this.constructor !== SFAbstractSingleSelect, "[SFAbstractSingleSelect] No implementation available for SFAbstractSingleSelect. You must subclass it.");
    assert(typeof modelOrArray == "undefined","[SFAbstractSingleSelect] Model or array required.");
    this._init(modelOrArray);
}

SFAbstractSingleSelect.prototype = (function() {
    return set(new Component(), {
        ...
        setModel : function(modelOrArray) {
            this._model = (modelOrArray.constructor == Array) ? new SFDefaultSingleSelectModel(modelOrArray) : modelOrArray;
            this._updateModel();
        },
        /**
        ...
        setOptions : function(options) {
            assert(options.constructor = Array, "[SFAbstractSingleSelect] New options must be an array.");
            this._model.clear();
        ...
        insertOptionAt : function(index, option) {
            if(this._model.addItemAt) {
               this._model.addItemAt(index, option);
            } else {
               this._model.insertItemAt(index, option);
            }
        },
        /**
