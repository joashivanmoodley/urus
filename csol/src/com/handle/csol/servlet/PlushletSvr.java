 ...
 */
public class PlushletSvr extends HttpServlet implements Protocol {

	/**
	 * 
	 */
	private static final long serialVersionUID = -5021892180839433869L;
	...
	public void init() throws ServletException {
		try {
			...
			// Start
			Log.info("init() Pushlet Webapp - version="
					+ Version.SOFTWARE_VERSION + " built=" + Version.BUILD_DATE);
			...
			throw new ServletException(
					"Failed to initialize Pushlet framework " + t, t);
		}
	}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			for (Enumeration e = request.getParameterNames(); e
				...
				event.setField(nextAttribute,
						request.getParameter(nextAttribute));
			}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			event = EventParser.parse(new InputStreamReader(request
					.getInputStream()));
		...
		try {
			...
			if (eventType.startsWith(Protocol.E_JOIN)) {
				...
				if (id == null) {
					response.sendError(HttpServletResponse.SC_BAD_REQUEST,
							"No id specified");
					Log.warn("Pushlet: bad request, no id specified event="
							+ eventType);
					return;
				}
			...
			Command command = Command.create(session, anEvent, request,
					response);
